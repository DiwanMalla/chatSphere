...
const HomePage = () => {
  ...
  return (
    <div className="h-screen bg-gray-900 text-gray-100 flex flex-col mt-165">
      {/* Header */}
      <header className="bg-gray-800 shadow-md py-4 px-6 flex justify-between items-center"></header>
      {/* Main Layout */}
          ...
          {isUsersLoading ? (
            ...
            <ul className="space-y-2">
              {users.map((user) => (
                <li
                  key={user.id}
                  onClick={() => useChatStore.getState().setSelectedUser(user)}
              ...
              <form
                onSubmit={(e) => {
                  ...
                  if (text) {
                    ...
                    useChatStore.getState().sendMessage(selectedUser.id, text);
